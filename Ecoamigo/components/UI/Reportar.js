...
import { reportarProblema } from '../../utils/api'
...
class Reportar extends React.Component{
  enviarReward = () => {
    ...
    reportarProblema(this.props.authedUser.id, this.problema)
      .then(res => {      
        this.props.dispatch(toggleLoading(this.props.loading))
      ...
      .catch(err => {
        this.props.dispatch(toggleLoading(this.props.loading))
        alert('Hubo un error, por favor intentalo nuevamente')
        console.error(err)
      })
  }
  render(){
        ...
        <ImageBackground source={Patron} style={{width: '100%', height: '100%', backgroundColor: CELESTE, flex: 1}}>
          ...
          <View style={styles.bloqueBlanco}>
            ...
            <View style={{backgroundColor: "rgba(0,0,0,.1)", width: '80%', paddingHorizontal: 20, paddingVertical: 15, maxHeight: 100, borderRadius: 25}}>
              <TextInput multiline={true} numberOfLines={3} onChangeText={(text) => this.problema = text} />
            </View>
