...
import { getReporteHistorico, getEcoZonal } from '../../utils/api'
...
class Reporte extends React.Component{
  ...
  componentDidMount(){
    ...
    const f = new Date()
    const fechaHoy = `${f.getFullYear()}-${parseInt(f.getMonth()) + 1 < 10 ? '0' + (parseInt(f.getMonth()) + 1) : parseInt(f.getMonth()) + 1}-${parseInt(f.getDate()) < 10 ? '0' + parseInt(f.getDate()) : f.getDate()}`
    this.fecha_hasta.defaultValue = fechaHoy
  }
  handleOnChange = () => {
    console.log(this.pais.value, this.provincia.value, this.zona.value, this.producto.value, this.fecha_desde.value, this.fecha_hasta.value)
    getReporteHistorico(this.pais.value !== "País" ? this.pais.value : false, this.zona.value !== "Zonas" ? this.zona.value : false, this.provincia.value !== "Provincia" ? this.provincia.value : false, this.producto.value !== "Producto" ? this.producto.value : false, this.fecha_desde.value !== "" ? this.fecha_desde.value : false, this.fecha_hasta.value !== "" ? this.fecha_hasta.value : false)
      .then(res => {
        res.data.success
          ? this.setState(() => ({
    ...
    return(
      <Container className="container-default background-default">
        <SubContainer>
          ...
          <button style={{maxWidth: "150px", alignSelf: 'flex-end'}} className="btn-form" onClick={() => DecargaTabla('tabla-reporte', 'ReporteHistórico')}>Descargar Doc</button>
          <FiltrosReporte>
            <Campos>
            <div style={{
                display: "flex",
                ...
                <div className="fecha" style={{paddingRight: "12px"}}>
                  <label className="labelForm">Fecha incial</label>
                  <div className="inputContainer">
                    <input className="inputSingle" required ref={(input) => this.fecha_desde = input} type="date"/>
                  </div>
                </div>
                <div className="fecha">
                  <label className="labelForm">Fecha final</label>
                  <div className="inputContainer">
                    <input className="inputSingle" required ref={(input) => this.fecha_hasta = input} type="date"/>
                  </div>
                </div>
              </div>
              <div>
                <div className="inputContainer" style={{marginTop: 0}}>
                  <select className="dropdown-select" required ref={(input) => this.pais = input}>
              ...
              <div>
                <div className="inputContainer">
                  <select className="dropdown-select" required ref={(input) => this.zona = input}>
              ...
              <div>
                <div className="inputContainer">
                  <select className="dropdown-select" required ref={(input) => this.provincia = input}>
              ...
              <div>
                <div className="inputContainer">
                  <select className="dropdown-select" required ref={(input) => this.producto = input}>
              ...
              <button style={{width: "100%"}} className="btn-form" onClick={() => this.handleOnChange()}>Buscar</button>
            </Campos>
            <ReporteContainer>
              <Paper>
