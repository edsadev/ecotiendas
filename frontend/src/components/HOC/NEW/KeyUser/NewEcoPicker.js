...
import { createEcoPicker, getEcotiendaForEcopicker } from '../../../../utils/api'
...
class NewEcoPicker extends React.Component {
  state = {
    ecotiendas: [],
    photo64: "",
    zonal_id: ""
  ...
  handleSubmit = (e) => {
    ...
    if (this.ecotienda.value !== 'Escoja'){
      createEcoPicker(this.ecotienda.value, this.cedula.value, this.nombre.value, this.apellido.value, this.direccion.value, this.genero.value, this.correo.value, this.state.photo64.result, this.fecha.value, this.celular.value, this.state.zonal_id)
      .then(res => {
        if(res.data.success === false){
          alert(res.data.mensaje)
          this.props.dispatch(toggleLoading(this.props.loading))
        } else {
          alert('EcoPicker creado')
          this.props.dispatch(toggleLoading(this.props.loading))
          ...
          <SubContainer>
            <Form>
              <Izq>
                ...
                <h3 className="title_green"><GoBackArrow classTo="arrow" history={this.props.history}/>Registrar EcoPicker</h3>
...
export default connect(mapStateToProps)(NewEcoPicker)